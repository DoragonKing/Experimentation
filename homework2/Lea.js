...
function draw(){
  ...
  fancyTriangles();}
    ...
    translate(random(-width,width),random(-height,height),0);
    fancybox();
  }
  else if(state==3){
   rotateY(random(0.1,2));
...
function fancysquares(){
  ...
  b=random(255);
  fill(r,g,b);
  var posx=random(-width,width);
  var posy=random(-height,height);
  rect(posx,posy,random(50),random(50));
}
function fancybox(){
  stroke(0)
